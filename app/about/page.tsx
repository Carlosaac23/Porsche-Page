import Image from 'next/image';

import Footer from '@/components/Footer';
import Header from '@/components/Header';
import ScrollToTopButton from '@/components/ScrollUp';
import { topImages, bottomImages } from '@/lib/images';

export default function AboutPage() {
  return (
    <>
      <ScrollToTopButton />
      <Header />
      <main className='my-16'>
        <section className='my-20'>
          <h2 className='mb-5 text-4xl xl:text-5xl'>The Icon of Performance</h2>
          <Image
            width={1500}
            height={500}
            className='mb-4 w-[90%] rounded-lg'
            src='/about-hero.webp'
            alt='A porsche 911 turbo s speeding'
          />
          <p className='text-sm font-light text-neutral-400 xl:text-lg'>
            The Porsche 911 Turbo S represents the pinnacle of precision engineering and timeless
            design, standing as one of the most accomplished sports cars ever built. Every detail
            has been carefully crafted to deliver an extraordinary balance between everyday
            usability and breathtaking performance. Its unmistakable silhouette reflects over five
            decades of Porsche heritage, refined through continuous evolution while remaining true
            to its roots.
          </p>
        </section>

        <section className='my-20'>
          <h2 className='mb-5 text-right text-4xl xl:text-5xl'>Unmatched Power</h2>
          <Image
            width={2000}
            height={1000}
            className='ml-auto w-[75%] rounded-lg'
            src='/about-design.webp'
            alt='A profile pic of porsche 911 turbo s'
          />
          <p className='mt-5 text-right text-sm font-light text-neutral-400 xl:text-lg'>
            At the heart of the 911 Turbo S lies a 3.8-liter, twin-turbocharged flat-six engine,
            producing 640 horsepower and 590 lb-ft of torque. Accelerating from 0 to 60 mph in just
            2.6 seconds, it sets the benchmark for supercar performance while maintaining everyday
            drivability.
          </p>
        </section>

        <section className='my-20'>
          <h2 className='mb-5 text-4xl xl:text-5xl'>Design That Inspires</h2>
          <div className='mx-auto grid grid-cols-1 gap-4 sm:grid-cols-[50%_50%] xl:w-5xl'>
            {topImages?.map(image => (
              <Image
                key={image.id}
                width={1000}
                height={500}
                className='w-full rounded-lg sm:h-96 sm:object-cover'
                src={image.src}
                alt=''
              />
            ))}
          </div>
          <div className='mx-auto mt-4 grid grid-cols-1 gap-4 sm:grid-cols-[30%_70%] xl:w-5xl'>
            {bottomImages?.map(image => (
              <Image
                key={image.id}
                width={1000}
                height={500}
                className='w-full rounded-lg sm:h-96 sm:object-cover'
                src={image.src}
                alt=''
              />
            ))}
          </div>
          <p className='mt-5 text-sm font-light text-neutral-400 xl:text-lg'>
            The 911 Turbo S combines iconic Porsche styling with modern aerodynamic refinements.
            Wide rear fenders, an adaptive spoiler, and striking LED lighting give it an
            unmistakable presence on the road. Inside, luxurious leather, carbon fiber accents, and
            advanced digital displays create a cockpit designed for both comfort and control.
          </p>
        </section>

        <section className='my-20'>
          <h2 className='mb-5 text-right text-4xl xl:text-5xl'>A Legacy of Innovation</h2>
          <Image
            width={1000}
            height={500}
            className='ml-auto w-[80%] rounded-lg'
            src='/about-timeline.webp'
            alt='A timeline of porsches 911 turbo s'
          />
          <p className='mt-5 text-right text-sm font-light text-neutral-400 xl:text-lg'>
            For decades, the Porsche 911 has defined the sports car segment. The Turbo S stands at
            the peak of this evolution, carrying forward a tradition of innovation, performance, and
            timeless design. Owning a 911 Turbo S is not just about driving a car — it’s about being
            part of a legacy.
          </p>
        </section>
      </main>
      <Footer />
    </>
  );
}
